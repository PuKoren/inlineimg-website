...
<html>
  <head>
    ...
    <style>
      ...
      h1 {
        color: #fff;
        background-image: linear-gradient(45deg, #9A409A, #d82424);
        padding: .6em .3em;
      }
      form {
        margin: auto;
      ...
      input[type=submit] {
        ...
        padding: .5em .7em;
        border: none;
        border-bottom: .2em solid #672b67;
        border-radius: 2px;
        font-size: 1em;
        font-weight: bold;
        color: #fff;
        cursor: pointer;
        background-color: #9A409A;
      }
  ...
  <body>
    ...
    <h2>Inline SVG, PNG, JPEG, GIF (and more) from an URL or from your disk</h2>
    <form id="form" method="POST" enctype="multipart/form-data">
      <div class="subform">
        <label for="remotefile">Use a remote image URL</label>
        <input id="remotefile" name="remotefile" type="text" value="" placeholder="Your online image URL" />
      </div>
      <div class="subform">
        <label for="userfile">Use an image from your disk</label>
        <input id="userfile" name="userfile" type="file" placeholder=""/></br>
      </div>
      <input type="submit" value="Inline now"/>
    </form>
    <?php if ($_SERVER['REQUEST_METHOD'] === 'POST') { ?>
    <div class="converted">
      <div class="converted__content">
      <?php
        try {
            $i = 'a';
            function print_tag($converter) {
                ...
                echo '<input onClick="this.select();" id="'.$i.'" type="text" value="'.$converted.'"/>';
                echo '<button class="btn" data-clipboard-target="#'.$i.'">Copy to clipboard</button>';
                echo '<p>You can use it in an image src="data" or in your css url("data")</p>';
